...
Changing the gamma parameters by factor 0.1 changes model characteristics considerably. Signal-noise ratio is well balanced in this case. The algorithm is able to detect the true parameter configuration.
...
Estimation errors are present, especially for slope estimates \gamma.
...
For very high signal-noise ratios, the algorithm performs poorly if negative scale slopes a involved in the model.
...
* Gamma-slopes need to be chosen sensibly, especially if multiple slopes are used.
* Negative gamma slopes lead to poorly behaving algortihm.
