...
# Abstract
This vignette documents the extensions of the `asp20model` package of the `asp20` repository [@R-asp20model] provided to us in the context of the course "Advanced Statistical Programming with R". 

The present package extension allows for estimation of location-scale regression models considering location ($\bm{\beta}$) and scale ($\bm{\gamma}$) parameters. 
For this purpose, we utilized a gradient boosting algorithm, which aims to improve a certain loss function. 
Boosting seems to be rather cumbersome for estimating location parameters alone, as it yields no advantage over OLS. However, for models with varying scale, OLS is no reliable estimation method. This is where gradient boosting plays its role. 
 
This algorithm optimizes the model parameter vectors ($\bm{\beta}$ and $\bm{\gamma}$) by fitting loss gradients. The goodness of fit of the gradient boosting algorithm was evaluated using simulated data. 
Results showed that for two-dimensional location-scale regression model, the `asp20boost` performed well, in multidimensional cases the estimates lack accuracy in two ways
...
It consists of a series of student projects that aim to estimate a location-scale regression model by implementing different methods each building upon a R6 class [@R-R6] given to us as part of the `asp20model` repository [@R-asp20model].
...
Next, in part 3  *software implementation* we explain how we implemented model and boosting algorithm in the `asp20boost` package and we offer a walkthrough regarding the basic usage of both the class and the function provided by it. 
Furthermore in part 4 *simulation studies* we present the results of  our simulation studies and assess the performance and the limitations of our package.
Finally, in section 5 *discussion* we discuss the results and limitation of this implementations and offer an outlook for future directions and development. 
...
The heteroskedastic response $y$ is plotted against the covariate $x$. In this case $x$ is used to model both location and scale, which allows for nice plotting.
...
Modeling the location is done by a conventional linear model. 
...
Transforming them using the link function $g(\sigma_i) = \log(\sigma_i)$, which is the inverse of the response function, allows for linearly regressing them on the scale predictor.
...
Gradient boosting is able to address this heteroskedasticity setting yielding correct variance estimates i.e. unbiased $\bm{\gamma}$ estimates.
...
It is an extension of the boosting concept, which enables for variable selection.
...
Possible choice for loss functions are, for example, the sum of least squares, penalized terms or non-linear ones.
This makes gradient boosting a very flexible method.
For location-scale regression, the negative log likelihood of the observed response is convenient as the loss function.  
...
For a derivation of the loss, see the appendix. 
...
The relevant gradients for boosting are attained by partially deriving the loss function by the unit-specific linear predictors $\eta_{\mu_i}$ and by $\eta_{\sigma_i}$.

\begin{equation}
 \begin{gathered}
 \bm{u}_{\mu} = \frac{\partial \text{loss}(\bm{y})}{\partial \eta_{\mu_i}} =  - \frac{y_i - \mu_i}{\sigma_i^2}
  \end{gathered}
\end{equation}
...
For a derivation of the gradients, see the appendix as well.
...
The gradients play the role residuals did in the last part:
...
However, this is a major issue, which we will discuss it later.
...
With this update restriction some components may become updated quite late in the progression of the algorithm execution.
This is the case when a particular component does not yield high improvements of model fit, which in turn reflects that this component, or predictive variable,  has a rather small effect on the response.
The algorithm is not executed until convergence of all parameter components, but stopped after a predefined number of iterations indicated by `maxit`.
...
## The `asp20boost` package

The code of the `asp20boost` package consists of two major parts.
The first is the R6 `LocationScaleRegressionBoost` class, which represents the location-scale regression model and includes some quantities (e.g. gradients) necessary for executing the boosting algorithm.
...
As already mentioned, gradient formulas are modified in order to make the algorithm working.
...
As the calculated gradients may be possibly of interest, we allowed for external access by setting `gradients_loglik_mu()` and `gradients_loglik_sigma()`as public methods.
...
The *conventional* version merely adds the gradients estimators to the active fields `beta` and `gamma`  multiplied by the defined step size. The `stepsize` argument is set by default to 0.01 for `beta`, and 0.1 for `gamma`.
...
First, we set the model by generating sample data and defining explanatory variables for the standard deviation of the response variable.
After setting the model `gradient_boost` will iterate over it and update the parameters at each step, depending on the selected method.
The function `update_parameter_componentwise()` will instruct the model to use componentwise boosting if set to `TRUE`. 
Depending on the sample data, a custom step size for `beta` and `gamma` might be useful, which can be set by means of the `stepsize` argument
`stepsize` is a vector of two entries, the first being the step size for beta updates, the second for gamma updates.
...
The number of iterations can be limited by specifying the desired value in the `maxit` argument.
...
To execute the conventional boosting algorithm on the model, the user can set the `componentwise = FALSE`, which will result in an update of all parameters at once.
See #5. 
...
This happens because of the huge standard deviations present in response $y$ that conceal the changes in the mean $\bar{y}$. 
...
To do this, we measure the gap between true values and estimates by means of the mean squared error (MSE).
...
In configuration 3, the `gradient_boost()` function leads to a higher MSE, than in configuration 1. 
...
While for two-dimensional location-scale regression settings, the `asp20boost` performed well, in multidimensional cases the estimates lacked accuracy in two ways:
   
   * Effect magnitudes were not estimated correctly.
   * Variable selection for scale parameters did not work well.

This problem might be addressed by testing varying the arguments `maxit` and `stepsize` given to the `gradient_boost()` function.

Another problem is the finding that for 51-dimensional parameter vectors, the algorithm did not work.
No variable becomes selected.
The algorithm only updates intercepts.
A thinkable solution might be to take different starting values for the parameter estimates $\bm{\hat{\beta}}$ and $\bm{\hat{\gamma}}$.

We also mentioned problems in the two-dimensional case, where one model with good signal-noise ratio was fit poorly.
Regarding this finding, we offered several explanations:
  
   * Further considerations to simulating data sensibly need to be taken, which we were not yet able to work out.
   * Our algorithm does not reliably yield robust results if negative $\bm{\gamma}$ slopes are included.
   * Our algorithm runs into problems if the signal is too strong compared to noise.

We mentioned that gradients calculated theoretically, and gradients implemented in the package do not match.
The theoretically derived ones are the following:
...
The practically implemented gradients are these:
...
Since, the algorithm works with the gradients we implemented, we conclude that these gradients are probably the correct ones.
Given this, our theoretical considerations on gradients and the loss function contain an error at some place.
The difference between theoretical and practical gradients is simply the factor $-1$.
For this reason, we consider this a minor problem.

What may be further developed if variable selection is ensured to work is the optimization of the number of iterations `maxit`.
This may be done by implementing cross validation.

To sum up, the `asp20boost` algorithm implementation performs well on two-dimensional cases, while lacking performance for multi-dimensional ones.
Specially, variable selection needs to be developed further, for example, by initializing parameter estimates before starting the `gradient_boost()` function or by further testing different combinations of step sizes and maximum iterations.

The simulation studies demonstrated clearly that gradient boosting is able to estimate location-scale regression and outperforms the calculations in the `asp20model` package.
