...
This algorithm optimizes the model parameter vectors ($\beta$ and $\gamma$) by fitting loss gradients. The goodness of fit of the gradient boosting algorithm was evaluated using simulated data. 
...
Next in part 3  *software implementation*, we explain how we implemented model and boosting algorithm in the `asp20boost` package and we offer a walkthrough regarding the basic usage of both the class and the function provided by the package. 
...
Finally, in section 5 *discussion*, we discuss the results and limitation of this implementations and offer an outlook for future directions for further development. 
...
For a derivation of the loss, see the appendix. The unit-wise gradients of this loss function play a key role. These are different from the gradients supplied in the `asp20model` package. 
...
The *conventional* version merely adds the gradients estimators to the active fields `beta` and `gamma`  multiplied by the defined stepsize. The `stepsize` argument is set by default to 0.01 for `beta`, and 0.1 for `gamma`.
...
While for two-dimensional location-scale regression model, the `asp20boost` performed well, in multidimensional cases the estimates lack accuracy in two ways
* Effect magnitudes are not estimated correctly.
* Variable selection for scale parameters does not work well.
This problem might be addressed by testing varying the arguments `maxit` and `stepsize` given to the `gradient_boost()` function.

Another problem is the finding that for 51-dimensional parameter vectors, the algorithm does not work.
No variable becomes selected, the algortihm only updates intercepts.
...
While the theoretically derived ones are the following
...
Since the algorithm works with the gradients we implemented, we conclude that these gradients probably are the correct ones.
Given this, our theoretical considerations on gradients and the loss function contain an error at some place.
The difference between theoretical and pratical gradients is simply the factor -1.
...
To sum up, the asp20boost algorithm performs well on two-dimensional cases, while lacking performance for multi-dimensional cases. Especially variable selection needs to be developed further, for example by initializing parameter estimates before starting the `gradient_boost()` function, or by further testing different combinations of stepsizes and maximum iterations.
