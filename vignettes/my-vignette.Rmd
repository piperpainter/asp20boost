...
It is important to emphasize that this package allows for the practical application of the concepts described in the „gradient boosting“and „componentwise boosting“ sections presented above. The concept of "residual boosting“ fulfilled an explanatory purpose and is not implemented implemented in the package.
...
The principal argument is the `model` object followed by several optional specifiers.
...
The class builds upon the `LocationScaleRegression` class of the `asp20model` package [@R-asp20model].
...
The *conventional* version merely adds the gradients estimators to the active fields $\bm{\beta}$ and $\bm{\gamma}$  adjusted by a learning rate of 0.1 for $\bm{\beta}$, and 0.01 for $\bm{\gamma}$.
...
*Does this relates to the last paragraph?*
This leads to the final implementation which consists of three main functions: `gradient_boost`, `update_parameters_compwise`, `update_parameters_conventional`.

  * `gradient_boost` - provides the boosting functionality for the model by doing `stepwise` calculations of the update parameters. Depending on the selected boosting method, the `update_parameters_compwise` or `update_parameters_conventional` function is called.
  ...
  * `update_parameters_conventional` - updates the beta and gamma parameters by the gradient estimators with respect to the predefined `stepsize`


## User Interface by example

First define the model by generating sample data and defining explanatory variables for the standard deviation of the response variable. After the definition `gradient_boost` will iterate over the model and update the parameters at each step depending on the selected method. The function parameter componentwise will instruct the model to use componentwise boosting if set to TRUE. Depending on the sample data a custom `stepsize` for beta and gamma might be useful, which can be set using the `stepsize` parameter by using a vector containing the `stepsize` ($\bm{beta}$, $\bm{\gamma}$).
